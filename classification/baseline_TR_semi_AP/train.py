...
parser.add_argument('--target_dir', type=str, default='../dataset/Breast-BUSI/train',
                    help='target domain')
parser.add_argument('--source_semi', type=str, default='../dataset/semi/UCLM')
...
### Output ###
parser.add_argument('--save_dir', type=str, default='./cp_UCLM2BUSI_TR_semi20_AP',
...
### Semi Dataset ###
source_semi_dataset = ImageDataset(args.source_semi, transform=train_transform(args.img_size))
source_semi_loader = DataLoader(source_semi_dataset, batch_size=args.bz, num_workers=4, shuffle=True, drop_last=True)
...
for step in tqdm(range(args.max_iters)):
    ...
    source_imgs_semi = batch_source_semi["img"].to(device)

    n_semi = source_imgs_semi.size(0)
    ########################

    ##### Style Transfer #####
    source_batch = torch.cat((source_imgs, source_imgs_semi), 0)
    ...
    logits = cls_model(cls_transform(Ics[n_semi:]))
    cls_loss = ce_loss(logits, batch_source_semi["target"].to(device))

    loss = loss_c + loss_s + cls_loss
    ...
    if step % args.log_interval == 0 or step == args.max_iters - 1:
        print("Train Step: [{}/{}] Loss: {:.4f} - Content: {:.4f} - Style: {:.4f} - Cls: {:.4f}".format(
            step, args.max_iters, 
    ...
    writer.add_scalar('loss_cls', cls_loss.item(), step + 1)
    ...
    if (step + 1) % args.test_interval == 0:
        ...
        test_dataset = ImageList(path_save, transform_w=cls_val_transform(256, 224)) # args.target_dir
