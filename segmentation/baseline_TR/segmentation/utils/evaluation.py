...
import segmentation.utils.metrics as metrics
from hausdorff import hausdorff_distance
from segmentation.utils.visualization import visual_segmentation, visual_segmentation_binary, visual_segmentation_sets, visual_segmentation_sets_with_pt
...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    sum_time = 0
    for batch_idx, (datapack) in enumerate(valloader):
        imgs = Variable(datapack['image'].to(dtype = torch.float32, device=args.device))
        masks = Variable(datapack['low_mask'].to(dtype = torch.float32, device=args.device))
        label = Variable(datapack['label'].to(dtype = torch.float32, device=args.device))
    ...
    iou_mean = np.mean(ious*100, axis=0)  # mean IoU

    # print("test speed", eval_number/sum_time)
    if opt.mode == "train":
        return dices, mean_dice, mean_hdis, iou_mean[1], val_losses
    else:
