...
class SourceEncoder(nn.Module):
    def __init__(self, img_size=256, patch_size=8, d_model=512, nhead=8, num_encoder_layers=3,
                 dim_feedforward=2048, dropout=0.1, activation="relu", num_prompts=1024):
        super().__init__()
        ...
        val = math.sqrt(6. / float(3 * reduce(mul, _pair(patch_size), 1) + d_model))  # noqa
        self.num_prompts = num_prompts
        self.deep_prompt_embeddings = nn.Parameter(torch.zeros(
                    num_encoder_layers, self.num_prompts, d_model))
    ...
    def forward(self, source_img):
        ...
        for idx, layer in enumerate(self.encoder_c):
            # Add positional encoding (broadcasted over batch) for each layers.
            source_feats = source_feats + self.pos_embedding  # shape: [num_patches, B, d_model]

            # Prompt embeddings (broadcasted over batch) for each layers.
            deep_prompt_emb = self.prompt_dropout(self.prompt_proj(
                self.deep_prompt_embeddings[idx]).unsqueeze(1).expand(-1, B, -1))            
            
            # Add prompt embeddings to the source features.
            fuse_feats = torch.cat((deep_prompt_emb, source_feats), dim=0)

            # Attention layer
            source_feats = layer(fuse_feats, pos=None)

            # Retain only the source tokens (first num_patches elements).
            source_feats = source_feats[self.num_prompts:, :, :]
...
class TokenDriven(nn.Module):
    ...
    def forward(self, source_feats, target_feats, pos=None):
        B = source_feats.shape[1]
        multi_targets = target_feats.view(-1, 1, 512).repeat(1, B, 1)

        ### Fusion of source and target features ###
        hs = source_feats
        for layer in self.decoder:
            hs = layer(hs, multi_targets, pos=pos)
